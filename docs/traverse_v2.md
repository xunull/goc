...
| `WithDefaultExclude()` | `WithSensibleDefaults()` |
| `WithExcludeUnknown()` | (TODO，目前没有等价项，可手动 `WithExcludeSuffix`) |
| `WithSyncMode()` | (TODO，目前 v2 始终并发；如果你真要单线程可设 `WithDirWorkers(1) + WithFileWorkers(1)`) |
| `WithProgressBarOut()` | (内置进度条暂未移植；自行用 `WithOnDirComplete` + 计数实现) |
